...
def jaccardIndexCalculate():
    ...
    for i in range(len(words_list)):
        if (i < 10):
            print('Jaccard index za:', i+1, 'mjesec i', i+2, 'mjesec:',
                  round(jaccardIndex(words_list[i], words_list[i+1]), 4))
            jaccard_list.append(
                round(jaccardIndex(words_list[i], words_list[i+1]), 4))

     # Create dataframe with months
    misec = pd.DataFrame(pd.date_range(
        start='01.06.2021', end='31.12.2021', freq='M').strftime('%m'), columns=['MJESEC'])
    #month_df['JACCARD INDEX'] = jaccard_list
    print(misec)
    # month_df.to_excel('./rezultati/jaccard_index.xlsx')
